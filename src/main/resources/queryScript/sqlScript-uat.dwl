...
ABDUNS AS DUNSNUMBER, ABNOE AS ABNOE, ABGROWTHR as ABGROWTHR, ABALKY as ABALKY, ABEFTB AS ABEFTB,ABAC08 AS ABAC08, TRIM(AICMGR) AS AICMGR, TRIM(AICPGP) as AICPGP,
...
TRIM(ABAC08) AS NBCAMKT, (SELECT DRDL01 FROM CRPCTL.F0005 WHERE DRSY = '01' AND DRRT = '08' AND TRIM(DRKY) = TRIM(T1.ABAC08)) AS NBCAMKTNAME,
((select max(w.aleftb) from crpdta.f0116 w where w.alan8 = t3.alan8)) AS EFFDATE, --//THIS WAS AN ISSUE WITH JOIN, FIXED ON LINE 89 BELOW
(SELECT DRDL01 FROM CRPCTL.F0005 WHERE DRSY = '01' AND DRRT = '08' AND ROWNUM = 1) AS NBCAMARKETDRKY,
(SELECT DRDL01 FROM CRPCTL.F0005 WHERE DRSY = '01' AND DRRT = '08' AND ROWNUM = 1) AS NBCAMKTDESDRKY,
(SELECT DRKY FROM CRPCTL.F0005 WHERE DRSY = '01' AND DRRT = '08' AND ROWNUM = 1) AS NBCAMKTDESDRDL01,
