...
DRDL01_ST AS (
    SELECT DRKY, DRDL01, ROW_NUMBER() OVER (PARTITION BY DRKY ORDER BY DRUPMJ DESC, DRUPMT DESC) AS rn
    FROM CRPCTL.F0005 
...
NTNADVPRCGRP AS (
	select DRDL01,DRKY, ROW_NUMBER() OVER (PARTITION BY DRKY ORDER BY DRUPMJ DESC, DRUPMT DESC) AS rn
	FROM CRPCTL.F0005
...
MailingName AS (
    SELECT WWAN8, WWMLNM, ROW_NUMBER() OVER (PARTITION BY WWAN8 ORDER BY WWUPMJ DESC, WWUPMT DESC) AS rn
    FROM CRPDTA.F0111 
    WHERE  WWIDLN = '0'
),
-- Subquery for vars.accountEngineer
accountEngineer AS (
    SELECT CUALPH3, CUAC36 --,ROW_NUMBER() OVER (PARTITION BY CUAC36 ORDER BY CUUPMJ DESC, CUUPMT DESC) AS rn
    FROM CRPDTA.F57DOLCU 
    WHERE  trim(CUDEPT) = 'Engineering'
),
-- Subquery for vars.accountMarketingRep
accountMarketingRep AS (
    SELECT CUALPH3, CUAC32 --, ROW_NUMBER() OVER (PARTITION BY CUAC32 ORDER BY CUUPMJ DESC, CUUPMT DESC) AS rn 
    FROM CRPDTA.F57DOLCU 
...
billAccount AS (
    SELECT AXDC,AXAN8
    FROM CRPDTA.F4780
      WHERE trim(AXXRTS) = 'SF' AND AXAN8 IN (select ABAN81 from CRPDTA.F0101 WHERE trim(ABAN8) != trim(ABAN81))
),
...
Website AS (
    SELECT EA.EAEMAL, EA.EAAN8, WW.WWAN8, ROW_NUMBER() OVER (PARTITION BY EA.EAAN8 ORDER BY EA.EAUPMJ DESC, EA.EAUPMT DESC) AS rn
...
billingCountryCode AS (
	SELECT DRDL02, DRKY, ROW_NUMBER() OVER (PARTITION BY DRKY ORDER BY DRUPMJ DESC, DRUPMT DESC) AS rn 
	FROM CRPCTL.F0005
...
ShippingAddress AS (
	SELECT AL1.*
	FROM CRPDTA.F0116 AL1
	WHERE AL1.ALAN8 IN ( select f1.ABAN8 from CRPDTA.F0101 f1 where f1.ABAT1 IN ('C','EU','EUX','CX','N') AND TRIM(f1.ABSIC) = 'NBCC') AND AL1.aleftb = (select max(w.aleftb) from crpdta.f0116 w where w.alan8 = AL1.alan8)
...
Parent AS (
    SELECT AXDC, AXAN8
    FROM CRPDTA.F4780
    WHERE trim(AXXRTS) = 'SF' AND AXAN8 IN (select ABAN86 from CRPDTA.F0101 WHERE trim(ABAN8) = trim(ABAN86))
),
-- Subquery for vars.CurrencyIsoCode
CurrencyIsoCode AS (
	SELECT f3.AICRCD,f3.AIAN8
	FROM CRPDTA.F03012 f3
	WHERE f3.AIAN8 IN (select f1.ABAN8 from CRPDTA.F0101 f1 where f1.ABAT1 IN ('C','EU','EUX','CX','N') AND TRIM(f1.ABSIC) = 'NBCC')
	--f1.ABUPMJ >= 124338 and f1.ABUPMT >= 143358  and (f3.AIUPMJ >= 124338 AND f3.AIUPMT >= 143358)
),
...
SELECT distinct
	...
	t23.DRDL02 AS billingCountryCodeDRDL02,
     	CASE 
		WHEN (TRIM(TO_CHAR(t22.ALCTR)) IS NOT NULL OR TRIM(TO_CHAR(t22.ALCTR)) != '') THEN TRIM(TO_CHAR(t22.ALCTR))
    	WHEN (TRIM(TO_CHAR(t22.ALCTR)) IS NULL OR TRIM(TO_CHAR(t22.ALCTR)) = '') AND TRIM(TO_CHAR(t1.ABSIC)) = 'NBCC' THEN 'CA'
    	ELSE 'US'
...
LEFT JOIN NBCAMKTDES t5 ON trim(t1.ABAC08) = trim(t5.DRKY) and t5.rn=1
LEFT JOIN ntnCustType t6 ON trim(t1.ABAC03) = trim(t6.DRKY) and t6.rn=1
LEFT JOIN NTNCustomerGroup t7 ON trim(t1.ABAC10) = trim(t7.DRKY) and t7.rn=1
LEFT JOIN NTNGlobalMarket t8 ON trim(t1.ABAC12) = trim(t8.DRKY) and t8.rn=1
LEFT JOIN Region t9 ON trim(t1.ABAC02) = trim(t9.DRKY) and t9.rn=1
...
LEFT JOIN billFax t17 ON trim(t1.ABAN81) = trim(t17.WPAN8) and t17.rn=1
LEFT JOIN billPhone t18 ON trim(t1.ABAN81) = trim(t18.WPAN8) and t18.rn=1
LEFT JOIN billFaxABAN8 t19 ON  trim(t1.ABAN81) = trim(t19.WPAN8)  and t19.rn=1
LEFT JOIN phone t20 ON trim(t1.ABAN8) = trim(t20.WPAN8)  and t20.rn=1
...
LEFT JOIN billingAddressType t28 ON trim(t27.AIBADT) = trim(t28.DRKY) and t28.rn=1
LEFT JOIN CreditManager t29 ON trim(t27.AICMGR) = trim(t29.DRKY) and t29.rn=1
LEFT JOIN customerPriceGroup t30 ON trim(t27.AICPGP) = trim(t30.DRKY) and t27.rn=1
LEFT JOIN dunBradstreet t31 ON trim(t27.AIDB) = trim(t31.DRKY) and t31.rn=1
LEFT JOIN HoldOrders t32 ON trim(t27.AIHOLD) = trim(t32.DRKY) and t32.rn=1
LEFT JOIN Payment t33 ON trim(t27.AITRAR) = trim(t33.PNPTC)
LEFT JOIN printMessage t34 ON trim(t27.AIINMG) = trim(t34.DRKY) and t34.rn=1
LEFT JOIN temporaryCreditMessage t35 ON trim(t27.AITSTA) = trim(t35.DRKY) and t35.rn=1
