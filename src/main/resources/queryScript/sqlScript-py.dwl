...
billFax AS (
    SELECT WPAR1, WPPH1, WPPHTP, WPAN8 
    FROM testdta.F0115 
    WHERE  trim(WPPHTP) = 'FAX'
),
-- Subquery for vars.billPhone
billPhone AS (
    SELECT WPAR1, WPPH1, WPPHTP, WPAN8
    FROM testdta.F0115 
    WHERE trim(WPPHTP) = 'BLANK'
),
...
billingAddress AS (
	SELECT AL1.ALAN8, AL1.ALEFTB, AL1.ALADD1, AL1.ALADD2, AL1.ALADD3, AL1.ALADD4, AL1.ALCTY1, AL1.ALADDS, AL1.ALADDZ, AL1.ALCTR
	FROM testdta.F0116 AL1
	WHERE AL1.ALAN8 in (select f1.ABAN81 from testdta.F0101 f1 where f1.ABUPMJ >= 124282 and f1.ABUPMT < 5066666666666666666666666666666666667 AND f1.ABAT1 IN ('C','EU','EUX','CX','N') AND TRIM(f1.ABSIC) = 'NBCC')
),
...
ShippingAddress AS (
	SELECT AL1.*
	FROM testdta.F0116 AL1
	WHERE AL1.ALAN8 in (select f1.ABAN8 from testdta.F0101 f1 where f1.ABUPMJ >= 124282 and f1.ABUPMT < 5066666666666666666666666666666666667 AND f1.ABAT1 IN ('C','EU','EUX','CX','N') AND TRIM(f1.ABSIC) = 'NBCC')
),
...
CurrencyIsoCode AS (
	SELECT AICRCD,AIAN8
	FROM testdta.F03012
	WHERE AIAN8 in (select f1.ABAN8 from testdta.F0101 f1 where f1.ABUPMJ >= 124282 and f1.ABUPMT < 5066666666666666666666666666666666667 AND f1.ABAT1 IN ('C','EU','EUX','CX','N') AND TRIM(f1.ABSIC) = 'NBCC')
),
-- Subquery for vars.F03012table
F03012table AS (
    select f2.ABAN8,f3.AIASN,f3.AIBADT,f3.AIACL,f3.AICMGR,f3.AICPGP,f3.AIDAOJ,f3.AIDLC,f3.AIARPY,f3.AIHOLD,f3.AITRAR,f3.AIINMG,f3.AITSTA,f3.AIAC06,f3.AIAC05,f3.AIDB 
	FROM testdta.F0101 f2
	LEFT OUTER JOIN testdta.F03012 f3
	ON trim(f2.ABAN8) = trim(f3.AIASN) 
	WHERE trim(f3.AIASN)  in (select f1.ABAN8 from testdta.F0101 f1 where f1.ABUPMJ >= 124282 and f1.ABUPMT < 5066666666666666666666666666666666667 AND trim(f1.ABAT1) IN ('C','EU','EUX','CX','N') AND TRIM(f1.ABSIC) = 'NBCC') 
...
branchCode AS (
    select AXEXRA, AXAN8
    from testdta.F4780
    where AXAN8 in (select f1.ABAN8 from testdta.F0101 f1 where f1.ABUPMJ >= 124282 and f1.ABUPMT < 5066666666666666666666666666666666667 AND f1.ABAT1 IN ('C','EU','EUX','CX','N') AND TRIM(f1.ABSIC) = 'NBCC')
),
...
-- we can optimise thise below two joins
...
where t1.ABUPMJ >= $(vars.jobRun.date) and t1.ABUPMT < $(vars.previousJobRun.time) AND t1.ABAT1 IN ('C','EU','EUX','CX','N') AND TRIM(t1.ABSIC) = 'NBCC' "